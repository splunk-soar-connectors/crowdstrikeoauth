...
def _get_incident_severity(fine_score: int) -> str:
    if fine_score >= 80:
        return "high"
    elif fine_score >= 60:
        return "medium"
...
def _create_incident_artifact(incident: dict) -> dict:
...
def _create_host_artifact(host: dict, incident: dict) -> dict:
    artifact = dict(_host_artifact_common)
    artifact["name"] = "Affected Host"
    artifact["source_data_identifier"] = (
        f"{incident.get('incident_id')}_{host.get('device_id')}"
    )
    artifact["severity"] = _get_incident_severity(incident.get("fine_score", 0))
...
def process_incidents(incidents: list) -> list:
    ...
    for incident in incidents:
        ...
        container["description"] = incident.get(
